...
class ModalEditPriceComponent extends Component {
  ...
  sendToken = async () => {
    console.log("valuewww===", this.state.selectedData);
    console.log("data Items===", dataItems[dataItems.length - 1].value);
    if (this.state.selectedData > dataItems[dataItems.length - 1].value) {
      alert(
    ...
    Keyboard.dismiss();
    this.props.onCallBackPrice &&
      this.props.onCallBackPrice(this.state.selectedData);
  };
  ...
  onBack = () => {
    ...
    this.props.onCallBackPrice && this.props.onCallBackPrice();
  };
