...
import { getEvtxContacts, shareTickets } from "../../../redux/actions/index";
...
const SentToTicketScreen = (props) => {
  const {
    navigation,
    getEvtxContacts,
    unEvtxContacts,
  ...
  const dataProps = props.navigation?.state?.params.dataProps;
  ...
  const onPressContact = useCallback(
    (item) => {
      if (!dataProps) return;
      let issuerIds = dataProps.map((s) => {
        issuerIds = s.tickets.map((s) => s.issuerId);
        return issuerIds;
      });

      issuerIds = issuerIds.flat();
      setIsLoadingData(true);
      shareTickets(issuerIds, item.id, onCallBack);
    },
    [props]
  );
  ...
  const onCallBack = (response) => {
    ...
    if (response.success) {
      Alert.alert("Alert", "Success!", [{ text: "OK", onPress: () => {} }], {
        cancelable: true,
      });
    }
  };
